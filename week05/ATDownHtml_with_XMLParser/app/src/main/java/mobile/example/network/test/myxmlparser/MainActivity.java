...
//        res/values/strings.xml 의 server_url 값을 읽어옴
    ...
    class NetworkAsyncTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            progressDlg = ProgressDialog.show(MainActivity.this, "Wait", "Downloading...");     // 진행상황 다이얼로그 출력(대기 상태 안내 다이얼로그)
        }

        @Override
        protected String doInBackground(String... strings) {  //이 부분만 별도의 쓰레드로 수행됨. MainActivity 같이 화면에 접근하는 항목 있으면 안됨!!
            String address = strings[0];
            String result = downloadContents(address);   //이거 뭔지 찾아보기
            if (result == null) {
                cancel(true);
