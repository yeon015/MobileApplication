...
//            poiList 의 POI 로 마커 추가 기능 수행
            ...
            for(POI poi : poiList) {
                try {
                    nowLatLng = new GeoReverseTask().execute(poi).get();  //https://ddolcat.tistory.com/46 -> get()

                    MarkerOptions options = new MarkerOptions();

                    options.position(nowLatLng);
                    Log.d(TAG, String.valueOf(nowLatLng.latitude));
                    options.title(poi.getTitle());
                    options.snippet(poi.getPhone());
                    options.icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE));

                    markerList.add(mGoogleMap.addMarker(options));
    ...
    class GeoReverseTask extends AsyncTask<POI, Void, LatLng> {
        Geocoder geocoder = new Geocoder(MainActivity.this);
        LatLng latLng = null;
        @Override
        protected LatLng doInBackground(POI... poi) {
            List<Address> addresses = null;
            try {
                addresses = geocoder.getFromLocationName(poi[0].getAddress(), 1);
        ...
        protected void onPostExecute(LatLng latLng) {
            ...
            nowLatLng = new LatLng(latLng.latitude, latLng.longitude);
            Log.d(TAG, String.valueOf(nowLatLng.latitude));
