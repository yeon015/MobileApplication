...
//    OnMapReadyCallback 선언 (지도 준비할 때 호출)
...
//                pOptions.color(Color.RED);
//                pOptions.width(5);
                //-> 여기 있으면 계속 새로운 객체 생성 되면서 선 그려지지 않음. 따라서 멤버 선언 해야함. 지도 준비하는 곳에서!

                // 현재 위치를 선 그리기 위치로 지정, 처음일 경우 시작점, n번째 경우 선으로 이어줌
                pOptions.add(latLng);
